...
the mentioned artifacts. In situations where only a sample from only one
invidiual is analyzed, we fall back on our standard filtering method using
certain parameters but in this case reinforced by pre-constructed
exclusionlist including information related to homo-polymers, repeat
regions, RNA editing sites, and known sequencing artifacts.
...
classication of homo/heterozygous mutations. The term used to describe this
phenomenon is heteroplasmy - which is defined as the presence of more than
one organellar genome within a single cell.

### Package goals
The primary goal of the package is to extract and analyze mitochondrial
variants from single-cell RNA-seq data. The method should also work
equally well with single-cell DNA-seq data. Although the majority of
functions provided by the package should work with BAM files from bulk
sequencing experiments, the majority of core functionality will be lost.

### Genome availability
There is built in compatiblity with the hg38/hg19 human genomes and
 mm10 mouse genome. 
...
reads located on the mitochondrial chromosome and also to depuplicate reads if
...
contains 5 to 8 columns where rows represent genomic positionsand columns
...
would like to count the number of nucleotide at each genomic position
from the `sites` parameter.


*    `sites` = This is the chromosome location that you will count 
...
it, reading in multiple BAM files at once will help speed things up. Default: 8.


*    `ignore_nonstandard` ignores bascalls that are not an A, G, C, T, or
...
*    `file` = This is a string containg the path for which you would like to
count the number of nucleotide at each genomic position from the `sites`
parameter for each cell barcode.


*    `sites` = This is the chromosome location that you will count 
variants from the default is the hg38 mitochondrial genome which has a length
of 16569 bp. Again it is very important that you provide the appropriate
chromosome names relative to your BAM file.
...
parameter will not provide a dramatic speed boost. Default: 8.
...
the most crucial step of the analysis. The prevelance of mitochondrial
...
### Calling mutations based on a exclusionlist   
...
The function `mutationCallsFromExclusionlist` performs fltering and
 then removes exclusionlisted variants. Parameters:
 
*  `lim.cov` is the minimum number of reads overlapping a site in a
 given single cell for it to be considered covered. The default value is 20.


*   `min.af `is the minimal allele frequency for a site to considered
...
*   `universal.var.cells` is the number of cells which must be mutant
...
setting for this 95% of all cells. However, again this parameter should be
adjusted based on the number and type of cells in a given analysis.


*   `min.af.universal` is the parameter used in conjunction with the
...
*   `min.num.samples` is the number of cells that must be considered
...
*   `exclusionlists` is the list of exclusionlists to use for filtering.
...
be excluded from the analysis. In the provided exclusionlist, we have `three`
which is stretchs of homopolymers on the mitochondrial chromosome of 3 bp or
greater length, `masked` which are regions masked in the Refseq or UCSC genome
releases (likely repetitive elements), regions identified from a previous study
(these can be excluded in most cases), and `rnaEDIT` which are RNA editing sites
from the REDIportal. **In all cases, these are relative to the 'hg38' genome
coordinates** and should not be used with other genomes without a liftOver step.
...
are only enriched in single patients. Parameters:


*   `MINREADS` is the minimum number of reads overlapping a site in
 a given single cell for it to be considered covered. The default value is 5.


*   `MINCELL` is the minimum number of total cells across all datasets that must
have **mutant** allele coverage over the target site. The default value is 20.


*   `MINFRAC` is the minimal allele frequency for a site to considered mutant
in an single The default value is 0.1.


*   `MINCELLS.PATIENT` is the minimum number of cells within an individual
patient that must have mutant coverage over the target site. The default
value is 10.


*   `MINRELATIVE.PATIENT` is the minimal allele frequency for a site to
be considered mutant in an single cell within a single patient. The default
value is 0.01.


*   `MINRELATIVE.OTHER`  is the minimal allele frequency for a site to be
considered mutant in an single cell within other patient (in contrast to
the previous parameter). If so, then the mutation is excluded as it is
considered shared between two more more samples/patients. The default
value is 0.1.
...
This may very based on the samples provided to function and thus may differ
from run to run if this is set to FALSE.
...
mitochondrial variants (i.e. they are indeed (sub)-clonal).
