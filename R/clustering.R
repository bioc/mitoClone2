...
varCluster <- function(mutcalls, fn = 0.1, fp = 0.02, cores = 1, time =10000, tempfolder = tempdir(), python_env = '',force_recalc = FALSE, method = 'SCITE') {
    ...
    if(method == 'PhISCS'){
        ...
        physics <- read.table(file.path(tempfolder, "out","in.CFMatrix"),header=TRUE, row.names = 1,sep="\t")
        cell.mutations <- physics
        txtCon <- textConnection(unique(apply(physics, 1, paste, collapse = ",")))
        clones <- read.csv(txtCon, header=FALSE , col.names = colnames(physics))
    ...
    if(method == 'SCITE'){
        ...
        write.table(x=mut.raw,file.path(tempfolder,"input_scite.txt"),sep=' ',row.names=FALSE,col.names=FALSE,quote=FALSE)##,sep=' ')
        write.table(mutation.names,file.path(tempfolder,"mut_scite_names.txt"),row.names=FALSE,col.names=FALSE,quote=FALSE)
        if (!file.exists(file.path(tempfolder,"scite_out_ml0.gv")) | force_recalc) {
            message("Ignoring multi-core param - SCITE uses a single thread")
            message("Now running the following command:", command)
            tryCatch(system(command), error = function(e) stop("SCITE error: ",e,"Make sure that the SCITE package is installed."))
        }else{
            message("Results found, skipping SCITE run. Warning! As SCITE will not overwrite previous results, if you are attempting to run this function again you may potentially need to delete your previous run or provide a new tempfolder.")
        } 
        scite.in <- readLines(file.path(tempfolder,"scite_out_ml0.gv"))
        ...
        scite.cells <- scite.in[seq(from=begin.si[2]+1,to=length(scite.in)-1)]
        ...
        mutcalls@tree <- addnodesSCITE(mutcalls@tree,scite.tree=scite.tree)
        ##mutcalls@tree <- clones2tree(clones)
        nodes.order <- getNodes(mutcalls@tree)[-1]
        flat.tree <- lapply(rapply(mutcalls@tree, enquote, how="unlist"), eval)
        flat.states.df <- unique(do.call(rbind,flat.tree[grep('states',names(flat.tree))]))
        row.names(flat.states.df) <- NULL
        clones <- as.matrix(flat.states.df)
        clone.names <- apply(flat.states.df[,nodes.order],1,function(x) max(which(x==1)))
        ##
        clone.names <- nodes.order[clone.names]
        clone.names[is.na(clone.names)] <- "root"
        ...
        scite.cells.multi.pre <- lapply(scite.cells.multi,function(x){
            x$X1 <- gsub("^Root$","root",x$X1)
            if(NROW(x) == 1){
                x.ret <- clones[x[1,]$X1,,drop=FALSE]
            }else{
                clone.reference <- clones[row.names(clones) %in% x$X1,]
                oldest.parent <- which.min(apply(clone.reference,1,function(z){
                    ...
                    raw.calls <- as.numeric(gsub("^\\?$","3",usedata[unique(x$real),colnames(clone.reference)]))
                    z <- z[!is.na(raw.calls)]
    ...
    ##   })
    ## }
    ref <- evaluate_likelihood(usedata[,colnames(cell.mutations)], cell.mutations)
    mutcalls@treeLikelihoods <- sapply(colnames(cell.mutations), function(node1) {
...
#'@export
quick_cluster <- function(mutcalls, binarize = FALSE,drop_empty = TRUE,  ...) {
...
#'@export
clusterMetaclones <- function(mutcalls, min.lik = 1, plot = TRUE) {
