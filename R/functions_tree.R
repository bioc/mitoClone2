...
addnodesSCITE <- function(tree,current=NULL,scite.tree=scite.tree){
    if(is.null(current)){
        current <- 'Root'
    }
    ...
    if(length(first.child) > 1){
        for ( i in 1:length(first.child) ){
            tree$children[[i]] <- list(states=tree$states,children=list(),mutation=scite.tree$end[first.child[i]])
        }
    }
    if(length(first.child) > 0){
        for ( i in 1:length(tree$children)) {
            tree$children[[i]] <- addnodesSCITE(tree$children[[i]], current=scite.tree$end[first.child[i]],scite.tree=scite.tree)
        }
    }
    return(tree)
}
...
topy <- function(tree, init=TRUE) {
  if (is.null(tree$child1) & is.null(tree$child2)) {
    ...
    out <- sprintf("Node('%s').%s.%s",tree$mutation,topy(tree$child1,FALSE),topy(tree$child2,FALSE) )
  } else if (is.null(tree$child2)) {
    out <- sprintf("Node('%s').%s",tree$mutation,topy(tree$child1,FALSE))
  } else {
