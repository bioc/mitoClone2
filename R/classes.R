...
mutationCallsFromMatrix <- function(M, N, cluster=NULL, metadata = data.frame(row.names = rownames(M)), binarize = 0.05) {
  ...
  out <- methods::new("mutationCalls", M=M, N=N, metadata = metadata, ternary=ternary, cluster=cluster)
  return(out)
}
...
#'@export
plotClones <- function(mutcalls, what = c("alleleFreq","ternary"), show = c(), ...) {
  what <- match.arg(what)
  if (what == "alleleFreq") plotData <- mutcalls@M / (mutcalls@M + mutcalls@N)
  ...
  if (length(show) > 1) annos <- data.frame(row.names = rownames(mutcalls@M), mutcalls@ternary[,show],
                      mutcalls@metadata)
  if (length(show) == 1) {
    annos <- data.frame(row.names = rownames(mutcalls@M), ann = mutcalls@ternary[,show],
                                               mutcalls@metadata)
    colnames(annos)[2] <- show
  }
  if (length(show) == 0)  annos <- data.frame(row.names = rownames(mutcalls@M), mutcalls@metadata)
  ...
  pheatmap::pheatmap(plotData, cluster_cols = FALSE, cluster_rows = FALSE, show_colnames = FALSE,
...
#'@export
getCloneLikelihood <- function(mutcall, mainClones =length(mutcall@mut2clone) > 0) mutcall@cell2clone
...
#'@export
getVarsCandidate <- function(mutcall) mutcall@cluster
...
getAlleleCount <- function(mutcall, type=c('mutant','nonmutant')){
    ...
    return(slot(mutcall,pulledslot))
}
